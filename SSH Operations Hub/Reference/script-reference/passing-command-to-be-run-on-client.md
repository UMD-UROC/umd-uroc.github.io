...
- Interactive commands, like `htop` or `vim`, are not supported
- Commands requiring user input will cause the script to hang
- For commands that require confirmation, like `apt upgrade`, use the `-y` flag when available to remove the requirement
  for user input:
